...
public class AdminDao extends PersonDao {
	...
	/**
	 * Save the Admin instance by storing it in your MySQL instance. This runs a
	 * INSERT statement.
	 */
	public Admin create(Admin user) throws SQLException {
		...
		try {
			...
			insertStmt.setTimestamp(2, new Timestamp(user.getLastLogin()
					.getTime()));
	...
	public Admin getAdminByUserName(String userName) throws SQLException {
		...
		try {
			...
			if (results.next()) {
				...
				Admin admin = new Admin(userName, person.getFirstName(),
						person.getLastName(), person.getEmail(),
						person.getPassword(), lastLoginDate);
				return admin;
			}
		} catch (SQLException e) {
			e.printStackTrace();
