...
public class ReviewCreate extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		request.getRequestDispatcher("/ReviewCreate.jsp").forward(request,
				response);
	}
	...
	protected void doPost(HttpServletRequest request,
		...
		for (Interest interest : interests) {
			...
			if (!rating.equals("Select"))
				interestRatings.put(interest.toString(),
		...
		for (Companion companion : companions) {
			...
			if (!rating.equals("Select"))
				companionRatings.put(companion.toString(),
						Integer.parseInt(rating));
		}
		...
		Iterator<Map.Entry<String, Integer>> iterator = interestRatings
				.entrySet().iterator();
		while (iterator.hasNext()) {
			Map.Entry<String, Integer> pair = (Map.Entry<String, Integer>) iterator
					.next();
			try {
				interestRatingDao.create(new InterestRating(pair.getValue(),
						Interest.valueOf(pair.getKey()), user, city));
		...
		CompanionRatingDao companionRatingDao = CompanionRatingDao
				.getInstance();
		Iterator<Map.Entry<String, Integer>> it = companionRatings.entrySet()
				.iterator();
		while (it.hasNext()) {
			Map.Entry<String, Integer> pair = (Map.Entry<String, Integer>) it
					.next();
			try {
				companionRatingDao.create(new CompanionRating(pair.getValue(),
						Companion.valueOf(pair.getKey()), user, city));
			} catch (SQLException e) {
				e.printStackTrace();
			}
		}
		...
		String success = "Thankyou. We value your ratings and review.";
		request.setAttribute("success", success);
		request.setAttribute("interests", Enumerations.Interest.values());
		request.setAttribute("companions", Enumerations.Companion.values());
		request.getRequestDispatcher("/ReviewCreate.jsp").forward(request,
